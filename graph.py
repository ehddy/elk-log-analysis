...
def minute_connect_category_plot(segment_data, column_name):
...
def minute_duration_category_plot(segment_data, column_name):
    # 차트를 그릴 데이터를 담을 빈 리스트 생성
    plot_data = []
    # 각 데이터프레임에 대해 그래프를 그리기 위한 데이터를 생성하여 plot_data 리스트에 추가
    for df in segment_data:
...
# 산점도 
def scatter_plot(data, column_1, column_2, label):
...
def describe_bar_chart(data, column, label):
...
def value_counts_top10_bar(data, column_name):
...
def value_counts_top10_bar_reverse(data, column_name):
    # 데이터셋의 'sHost' 열 값 빈도수 계산
    top_hosts = data[column_name].value_counts()[:10]
...
def groupby_mean_plot(data, group_name, mean_column_name):
...
def seasonal_decompose_plot(data, period=5):
